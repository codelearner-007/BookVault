...
description: Enforces repository architecture and coding rules (Next.js + FastAPI + Supabase) with a short implementation and review checklist. Use when building features, refactoring, adding endpoints/services, touching DB/migrations, or when the user asks to “follow our architecture/rules/best practices”.
---
...
- Apply the **Golden Rule** end-to-end:
  - **Auth flows**: Component → Service/Hook → **Next.js API Route** → Supabase Auth (SSR client) → Cookies
  - **All other app features** (DB/business logic): Component → Service → **FastAPI** → Database (Supabase/Postgres)
...
- **Pages are thin**: `frontend/src/app/**/page.tsx` should mostly render a composed component (target 30–50 lines).
- **Zero duplication**: never clone pages for roles. Reuse one page/component with role-based logic.
- **Service layer required**: components call `frontend/src/lib/services/**` for API calls.
- **Supabase client rules**:
  - **Auth-only exception**: Supabase client is allowed only in `frontend/src/app/api/auth/**/route.ts` and middleware/session utilities.
  - **Non-auth features**: Supabase/Postgres access must happen in **FastAPI** (not in Next.js API routes, components, hooks, or services).
...
- **Avoid `db reset`** unless explicitly needed (it discards local data). Early development exception: if you are actively editing existing migrations, `db reset` is expected.
...
- [ ] No Supabase client in components/services (auth API routes only)
- [ ] Auth-only Next.js API routes; all other business logic in FastAPI backend
